...
multiFile:
multiMedia:
description: 小满 React 教程「学习React【Router】NavLink」笔记。素材来源 message163.github.io/react-docs。
笔记类型: 收集笔记
阐述日期:
tags:
  - React
...
> 素材来源：[小满 React 文档](https://message163.github.io/react-docs/react/router/apis/navlink.html)
...
Navlink 会根据当前路由和 to 属性是否匹配，自动激活。react-router 会为其自动添加样式：
...
> 注意：
> 1. `viewTransition` 需要谷歌 111 版本才能使用，注意兼容性。
> 2. `pending` 只有数据模式（data router）和框架模式才能使用，声明式路由（`<BrowserRouter>` + `<Routes>`）不能使用。
