...
- [ ] 每天按日期整理的ai笔记功能有了，现在需要增加整理一个对话，或者上下文中的内容到制定的笔记中的功能，并列出基础知识点和官网文档地址。 [🔗Dida](obsidian://dida-task?didaId=6a1909b8e4b037c374061c07)
  - ⤷ 2026-06-26 复现导向改版已部分落地：Layer 2「复现主题篇」即「把会话内容整理进指定主题笔记」，基础知识点/官网文档并入复现篇的 原理/踩坑/延伸阅读。剩余：把"单条对话 / 选中上下文 → 指定笔记"做成可手动触发的入口
- [ ] ① **水位线增量整理**：在 AI笔记目录维护 `.organized.json`（会话ID → 已整理到的消息时间戳），提示词从"按 -mtime 0 扫描"改为"整理所有未整理消息"——一次解决重复追加、隔天遗漏、跨天归属三个问题
- [ ] ② **复习闭环**：整理时自动将 `阐述日期` 设为 +7 天（进入 Alt+X 复习流）；复现篇/日笔记知识点输出 `[[wikilink]]` 连接概念页（涉及格式变更，先出样例确认）
...
- **目标与形态（2026-06-26 复现导向改版）**：AI 笔记目标是「方便之后复现操作」，**不再原文照搬**。两层产出：
  - **Layer 1 · 日 AI 笔记**（`0-收集箱/AI笔记/YYYY-MM-DD.md`，`笔记类型: AI整理` + `cssclasses: ai-note` + 固定 `AI笔记` 主题）= 轻量捞取/索引层：每会话只记 项目/范围 → 做了什么 → 关键结论 → 改动文件 → 一句话坑，并 `[[链接]]` 到复现篇
  - **Layer 2 · 复现主题篇**（`<领域>/收集/<主题>.md`，普通 `收集笔记` + 对应领域主题）= 真正交付物：按 原理 → 完整代码 → 配置 → 复现Checklist → 踩坑 → 文件清单 组织（模板：`学习领域/收集/MinIO 视频直传.md`）。唯一逐字保留的是**代码**
...
- **Markdown 安全**：代码一律加 ``` 围栏或行内反引号，禁止裸 `<...>`（Obsidian 会当 HTML 破坏后续渲染）
- **过大先列计划**：列主题清单问用户哪些出复现篇、哪些仅捞取，不静默截断
...
- [x] 整理规则制定并收敛到 CLAUDE.md ✅ 2026-05-28（05-29 修正，06-12 规则 v2，06-26 规则 v4）
  - 06-12 新增：敏感信息脱敏、多用户机器扫描范围、jsonl 提取排除项、卡片盒格式强制
  - 06-26 规则 v4（复现导向改版）：AI 笔记目标改为「方便复现操作」，两层产出（日笔记捞取层 + 复现主题篇），废弃「原文照搬 + 知识点解析」，模板见 `学习领域/收集/MinIO 视频直传.md`
...
> [ ] ② 复习闭环：阐述日期 +7 天 + 知识点 `[[wikilink]]`（先出样例确认格式）
...
3. 写**日 AI 笔记（捞取层）**：每会话简记 做了什么/关键结论/改动文件/一句话坑，**不再原文照搬**；代码一律加 ``` 围栏、禁止裸 `<...>`；密码/token 自动脱敏
4. 对有复现价值的会话，提炼独立**复现主题篇**到 `<领域>/收集/<主题>.md`（原理→完整代码→配置→复现Checklist→踩坑→文件清单），并在日笔记中 `[[链接]]`
5. 日笔记输出到 `0-收集箱/AI笔记/YYYY-MM-DD.md`（已存在则追加），主题固定挂 `AI笔记` 并向 `AI笔记.canvas` 追加节点
6. 写入后自检并报告：脱敏残留 0、围栏外裸 `<...>` 0、canvas 节点已追加
...
### 输出笔记的结构（现行版，2026-06-26 复现导向改版）

**两层产出：**

```
Layer 1 — 0-收集箱/AI笔记/2026-06-26.md（捞取/索引层，轻量）
├── frontmatter（笔记类型: AI整理 / cssclasses: ai-note / 主题固定 AI笔记）
├── ## 2026-06-26（标题 = 文件名）
...
│   ├── 一句话踩坑
│   └── → 复现篇：[[<领域>/收集/<主题>]]（若有）
└── ### 会话二：……（纯问答/概念无复现价值则仅列知识点）

Layer 2 — <领域>/收集/<主题>.md（复现主题篇，真正交付物，模板见 MinIO 视频直传）
├── frontmatter（笔记类型: 收集笔记 / 对应领域主题，非 AI笔记）
├── 引言 blockquote（定位 + 来源项目/技术栈 + 脱敏声明）
├── ## 一、原理（目标 + 流程图）
├── ## 二、代码（完整、可直接复制，逐字不裁剪）
├── ## 三、配置 / 命令
├── ## 四、复现 Checklist [ ]
...
- 以「能否照着复现」为标准提炼，**不再原文照搬**；唯一逐字保留的是代码（复现篇里完整、不裁剪）
