...
2026-09-02 至 2026-09-03 已跑通一轮“新加坡运动户外选品 → 1688 找货与核价 → 妙手公用采集箱 → TikTok 采集箱 → 目标店铺校正”，并记录了失败路径和页面陷阱。

新对话除本计划外，必须继续读取：

```text
$TK_PROJECT_ROOT/automation/选品采集上品-实战复盘与批量执行SOP.md
$TK_PROJECT_ROOT/automation/runs/<RUN_ID>/run-log.md
```

实战 SOP 包含：成功采集路径、OpenCLI 动态 ref 处理、1688 运费读取、品牌/附件低价陷阱、妙手错误默认店铺修正、批量状态机、幂等查重规则和可直接复制的新对话提示词。
