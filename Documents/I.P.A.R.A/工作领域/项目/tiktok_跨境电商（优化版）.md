...
multiFile:
multiMedia:
description: 完整保留原始87张操作截图，并结合课程逐字稿和项目资料优化TikTok东南亚选品、核价、上架、营销、数据复盘与订单履约流程。
笔记类型: AI整理
阐述日期:
tags:
  - TikTok
...
> 用于重新走通一次「选品 → 核价 → 上架 → 获客 → 履约 → 复盘」。以 [[tiktok_跨境电商|原始实操笔记]] 为主线，补充课程原理与项目资料，保留关键截图。原笔记和原始资料不变；账号、密码、收件信息不在本笔记中记录。

> [!warning] 使用边界
> 课程日期为 **2026-08-28**，资料核对日期为 **2026-08-31**。本文中的费率、汇率、物流价格和销售数据均须区分“资料快照”与“当前经营参数”。课程示例不是平台统一标准，核价结果不是实际结算利润。本文没有登录店铺后台核验实时费用或履约期限。
...
```mermaid
flowchart TD
    A[确认店铺、类目与供应链] --> B[FastMoss 找候选商品]
    B --> C[1688 核实货源、规格、包装和授权]
    C --> D{各 SKU、各国家核价通过？}
    D -- 否 --> B
    D -- 是 --> E[妙手采集、编辑、人工审核]
    E --> F[Seller Center 核对发布与价格]
    F --> G[促销、达人合作、内容测试]
    G --> H[订单采购、云仓发货、售后]
    H --> I[按订单结算复盘利润]
    I --> B
...
| TikTok Shop Seller Center | 确认真实上架状态、站点价格、活动、联盟、订单和结算 |
...
> [!info] 阅读方式
> 本节按原笔记的操作先后保留 **87 张截图和全部主要步骤**，用于跟着界面连续操作；文字只做纠错、补充过渡和脱敏。后续章节解释为什么这样做、核价怎么算，以及哪些课程操作不能直接照搬。平台界面更新后，以当前后台名称和提示为准。
...
原笔记在这里结束，没有继续记录采购单号、国内快递单号、云仓处理和取消订单的衔接。完整补充流程见 [[#十、订单采购与云仓履约]]。

> [!success] 图文实录已完整保留
> 到这里，原笔记的 87 张截图和主要操作步骤已按原顺序全部呈现。下面进入优化分析：补充选品判断、核价推导、内容合规、数据诊断和履约检查，避免只会跟着界面点击而不知道如何判断结果。
...
课程先演示跟新品，后面补充了供应链与标杆店铺方法。不要只看“卖了多少单”，还要回答“它为什么能卖，我们具备哪些条件”。〔课程 00:03:04–00:10:59；01:01:49–01:14:59〕
...
2. **新品榜／商品搜索：** 选国家、分类、店铺类型；已有明确商品时直接搜索。
...
参考原笔记的 [跨境 ERP 助手说明](https://erp.91miaoshou.com/help_center/article_3250.html)：在 1688 商品页采集 → 进入妙手采集箱 → 关联正确的 TikTok 店铺与国家 → 编辑商品。使用当前官方渠道提供的插件版本，项目中的安装包只作存档，不默认是最新版本。
...
先看待履约、取消退款和仓库异常，再看商品异常与活动价格，然后处理达人回复、选品和内容测试。每个完成结算的订单都可用于修正核价假设；不要只统计上架数和 GMV。
