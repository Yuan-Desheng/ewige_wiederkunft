...
网络名称:JZSD2-5G
密码:jzsd8888
http://192.168.66.160:8791/
网络名称:JZSD-5G

密码:jzsd8888
http://192.168.1.125:8791/
tailscale访问地址
http://100.98.66.122:8791/
...
```
修改对标库
1.类型单选的顺序是：全部、爆款、非爆款（默认选择爆款）；
2.状态也修改组件为何类型、带货一样的单选框
3.去掉筛选栏的排序，修改表格的点击表头的排序不是只拍本页面的，而是调用接口去实现真正的排序
4.操作列有点窄，表格整体不要限制最大宽度
5.检查一下样式是否还有需要优化的地方

优化页面右上角的主题切换图标，增加动效，并且增加圆圈渐变动画
```
