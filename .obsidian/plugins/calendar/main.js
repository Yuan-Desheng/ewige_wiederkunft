...
async function computeSHA256Hex(message) {
    try {
        ...
        console.error("[Calendar] computeSHA256Hex failed", e);
        return null;
    }
}
...
class CalendarSettingsTab extends obsidian.PluginSettingTab {
    ...
    addWeekStartSetting() {
        ...
        // 兼容部分环境下 _bundledLocaleWeekSpec 不存在的情况，避免抛错导致整个设置页不渲染
        let localeWeekStartNum = void 0;
        if (window._bundledLocaleWeekSpec && typeof window._bundledLocaleWeekSpec.dow === "number") {
            localeWeekStartNum = window._bundledLocaleWeekSpec.dow;
        }
        else if (moment.localeData && moment.localeData() && typeof moment.localeData()._week?.dow === "number") {
            localeWeekStartNum = moment.localeData()._week.dow;
        }
        else {
            // 回退到周日（0），保证始终有一个合法值
            localeWeekStartNum = 0;
        }
        const localeWeekStart = moment.weekdays()[localeWeekStartNum];
        new obsidian.Setting(this.containerEl)
            .setName("本周开始时间：")
            .setDesc("选择星期几开始。选择“本地化默认值”以使用 moment.js 指定的默认值。")
    ...
    addConfirmCreateSetting() {
        new obsidian.Setting(this.containerEl)
            .setName("创建新笔记前是否需要确认")
            .setDesc("创建新笔记前显示确认弹窗（建议关闭，以配合其他插件使用）")
            .addToggle((toggle) => {
    ...
    addLocaleOverrideSetting() {
        ...
        new obsidian.Setting(this.containerEl)
            .setName("覆盖区域设置：")
            .setDesc("如果您想使用与默认语言环境不同的语言环境，请设置此项。")
            .addDropdown((dropdown) => {
    ...
    addWeatherSettings() {
        ...
        new obsidian.Setting(this.containerEl)
            .setName("启用天气显示")
            .setDesc("在日历单元格中显示天气预报（未来 15 天）")
            .addToggle((toggle) => {
            toggle.setValue(this.plugin.options.weatherEnabled);
            toggle.onChange(async (value) => {
                this.plugin.writeOptions(() => ({
        ...
        new obsidian.Setting(this.containerEl)
            .setName("位置 (city / location id)")
            .setDesc(createFragment((frag) => {
            frag.appendText("填写城市名或位置ID，例如 beijing或者北京（可选填，若不填，则通过IP定位自动获取），点击查看或下载");
            const link = frag.createEl("a", {
                text: "城市列表",
                href: "https://tp-static.oss-cn-hangzhou.aliyuncs.com/download/data/thinkpage_cities%2020230505.xls.zip",
            });
            link.setAttr("target", "_blank");
        }))
            .addText((textfield) => {
            textfield.setPlaceholder("例如: beijing 或 你的城市 ID");
            textfield.setValue(this.plugin.options.weatherLocation || "");
            textfield.onChange(async (value) => {
                this.plugin.writeOptions(() => ({
...
function add_css$4() {
	...
	style.textContent = ".day.svelte-q3wqg9{background-color:var(--color-background-day);border-radius:4px;color:var(--color-text-day);cursor:pointer;font-size:0.8em;height:100%;padding:1px;position:relative;text-align:center;transition:background-color 0.1s ease-in, color 0.1s ease-in;vertical-align:baseline;overflow:hidden}.day.svelte-q3wqg9::before{content:attr(data-weather-icon);position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:1.4em;opacity:0.7;z-index:0;pointer-events:none;user-select:none}.day.svelte-q3wqg9:hover{background-color:#498d6aa3}.day.active.svelte-q3wqg9:hover{background-color:#498d6aa3}.adjacent-month.svelte-q3wqg9{opacity:0.25}.today.svelte-q3wqg9{color:var(--text-error)}.day.svelte-q3wqg9:active,.active.svelte-q3wqg9,.active.today.svelte-q3wqg9{color:var(--text-on-accent);background-color:#498d6aa3}.dot-container.svelte-q3wqg9{display:flex;flex-wrap:wrap;justify-content:center;line-height:6px;min-height:6px}.day-number.svelte-q3wqg9{position:relative;z-index:15}.weather-text.svelte-q3wqg9{position:relative;font-size:0.5em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;z-index:1}";
	append(document.head, style);
}
...
function create_default_slot$1(ctx) {
	...
	return {
		c() {
			...
			t2 = space();
			div2 = element("div");
			attr(span0, "class", "day-number svelte-q3wqg9");
			attr(div0, "class", "dot-container svelte-q3wqg9");
			attr(div2, "class", "weather-text svelte-q3wqg9");
		...
		m(target, anchor) {
			...
			append(div1, t2);
			append(div1, div2);
			div2.textContent = "" + (/*metadata*/ ctx[7].weatherLabel || "");
...
async function metadataReducer(promisedMetadata) {
    ...
    const merged = metas.reduce((acc, meta) => {
        const next = {
            ...
            weatherLabel: acc.weatherLabel || meta.weatherLabel || "",
            weatherIcon: acc.weatherIcon || meta.weatherIcon || "",
        };
        return next;
    }, baseMeta);
    return merged;
...
function mapWeatherTextToIcon(text) {
    if (!text)
        return "⛅";
    if (text.indexOf("雷") !== -1)
        return "⛈️";
    ...
    if (text.indexOf("雨") !== -1)
        return "🌧️";
    if (text.indexOf("雾") !== -1 || text.indexOf("霾") !== -1 || text.indexOf("沙") !== -1)
        return "🌫️";
    if (text.indexOf("云") !== -1 || text.indexOf("阴") !== -1)
        return "☁️";
    if (text.indexOf("晴") !== -1)
        return "☀️";
    return "⛅";
...
const WEATHER_CACHE_TTL = 1000 * 60 * 30;
const WEATHER_CACHE_STORAGE_KEY = "calendar-weather-cache-v1";
const WEATHER_CLOUD_AUTH_URL = "https://b.ytmaps.vip/wt_fff20251210";
let weatherCache = {
    lastFetchTime: 0,
    lastFetchDate: "",
    byDate: {},
...
async function fetchAuthFromCloud() {
    if (!WEATHER_CLOUD_AUTH_URL) {
        return null;
    }
    try {
        const res = await fetch(WEATHER_CLOUD_AUTH_URL);
        if (!res || !res.ok) {
            return null;
        }
        const json = await res.json();
        if (!json || !json.authStr || !json.sig) {
            return null;
        }
        return { authStr: json.authStr, fixedSig: json.sig };
    }
    catch (e) {
        return null;
    }
...
async function fetchWeatherForecast() {
    ...
    const now = Date.now();
    const todayStr = window.moment().startOf("day").format("YYYY-MM-DD");
    if (!weatherCache.lastFetchDate && !weatherCache.lastFetchTime) {
        try {
            const raw = window.localStorage && window.localStorage.getItem(WEATHER_CACHE_STORAGE_KEY);
            if (raw) {
                const saved = JSON.parse(raw);
                if (saved && typeof saved === "object" && saved.byDate && typeof saved.byDate === "object") {
                    weatherCache.byDate = saved.byDate || {};
                    weatherCache.lastFetchTime = typeof saved.lastFetchTime === "number" ? saved.lastFetchTime : 0;
                    weatherCache.lastFetchDate = typeof saved.lastFetchDate === "string" ? saved.lastFetchDate : "";
                }
            }
        }
        catch (e) {
    ...
    if (weatherCache.fetching) {
        try {
            await weatherCache.fetching;
    ...
    if (weatherCache.lastFetchDate === todayStr && weatherCache.byDate && Object.keys(weatherCache.byDate).length) {
        return weatherCache.byDate;
    }
    if (weatherCache.lastFetchTime && now - weatherCache.lastFetchTime < WEATHER_CACHE_TTL && weatherCache.byDate && Object.keys(weatherCache.byDate).length) {
        return weatherCache.byDate;
    }
    const auth = await fetchAuthFromCloud();
    if (!auth) {
        return weatherCache.byDate || {};
    }
    const { authStr, fixedSig } = auth;
    const startDay = window.moment().startOf("day");
    const days = 15;
    const language = "zh-Hans";
    const unit = "c";
    const fetchByLocation = (rawLocation) => {
        if (!rawLocation) {
            return Promise.resolve({});
        }
        const location = encodeURIComponent(rawLocation);
        const url = `https://api.seniverse.com/v3/weather/daily.json?${authStr}&sig=${fixedSig}&location=${location}&language=${language}&unit=${unit}&start=0&days=${days}`;
        return fetch(url)
            .then((res) => res.json())
            .then((json) => {
                const map = {};
                if (json && Array.isArray(json.results) && json.results[0] && Array.isArray(json.results[0].daily)) {
                    json.results[0].daily.forEach((item) => {
                        if (!item || !item.date) {
                            return;
                        }
                        const key = item.date;
                        const text = item.text_day || item.text || "";
                        const high = item.high || item.high_temperature;
                        const low = item.low || item.low_temperature;
                        let label = text;
                        ...
                        const icon = mapWeatherTextToIcon(text);
                        map[key] = {
                            label,
                            icon,
                        };
    ...
    const fetching = (async () => {
        ...
        if (opts.weatherLocation) {
            ...
            if (!map || !Object.keys(map).length) {
                const fallbackMap = await fetchByLocation("ip");
                if (fallbackMap && Object.keys(fallbackMap).length) {
                    map = fallbackMap;
                }
            }
        }
        ...
        if (map && Object.keys(map).length) {
            ...
            try {
                const payload = {
                    lastFetchTime: weatherCache.lastFetchTime,
                    lastFetchDate: weatherCache.lastFetchDate,
                    byDate: weatherCache.byDate,
                };
                window.localStorage && window.localStorage.setItem(WEATHER_CACHE_STORAGE_KEY, JSON.stringify(payload));
    ...
    weatherCache.fetching = fetching;
    return fetching;
}

async function getWeatherLabelForDate(date) {
    const opts = get_store_value(settings) || {};
    if (!opts.weatherEnabled) {
        return "";
    }
    ...
    // 仅从当前会话的内存缓存读取，避免使用上一会话遗留的本地存储数据
    const info = weatherCache.byDate && weatherCache.byDate[key];
    if (!info || !info.label) {
        return null;
    }
    return info;
...
const weatherSource = {
    getDailyMetadata: async (date) => {
        const info = await getWeatherLabelForDate(date);
        if (!info || !info.label) {
            return {
                dots: [],
            };
        ...
        return {
            ...
            dataAttributes: info.icon
                ? { "data-weather-icon": info.icon }
                : {},
        };
    },
    getWeeklyMetadata: async (_date) => {
        return {
            dots: [],
        };
    },
...
async function getWeatherLabelFromStorageForDate(date) {
    if (!date || !date.clone || !window.moment) {
        return "";
    }
    const key = date.clone().startOf("day").format("YYYY-MM-DD");
    if (weatherCache.byDate && weatherCache.byDate[key] && weatherCache.byDate[key].label) {
        return weatherCache.byDate[key].label;
    }
    try {
        const raw = window.localStorage && window.localStorage.getItem(WEATHER_CACHE_STORAGE_KEY);
        if (!raw) {
        ...
        const saved = JSON.parse(raw);
        const byDate = saved && typeof saved === "object" && saved.byDate && typeof saved.byDate === "object"
            ? saved.byDate
            : null;
        if (!byDate || !byDate[key] || !byDate[key].label) {
            return "";
        }
        return byDate[key].label;
    }
    catch (e) {
        return "";
    }
...
class CalendarView extends obsidian.ItemView {
    ...
    async writeWeatherToDailyNote(file, date) {
        try {
            const label = await getWeatherLabelFromStorageForDate(date);
            if (!label) {
                return;
            }
    ...
    async openOrCreateDailyNote(date, inNewSplit) {
        ...
        if (!existingFile) {
            ...
            tryToCreateDailyNote(date, inNewSplit, this.settings, (dailyNote) => {
                ...
                this.writeWeatherToDailyNote(dailyNote, date);
            });
            return;
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        activeFile.setFile(existingFile);
        this.writeWeatherToDailyNote(existingFile, date);
    }
}
...
class CalendarPlugin extends obsidian.Plugin {
    ...
    async verifyContentProtectionHash(settings) {
        ...
        try {
            const payloadParts = [
                settings.deviceId || "",
                settings.isActivated ? "1" : "0",
                settings.hasBeenActivated ? "1" : "0",
            ];
            const payload = payloadParts.join("|");
            const hex = await computeSHA256Hex(payload);
            if (!hex) {
                return false;
            }
            return hex === settings.hash;
        }
        catch (e) {
            console.error("[Calendar] verifyContentProtectionHash failed", e);
            return false;
        }
    }
    async onload() {
        const isContentProtectionActivated = await this.checkContentProtectionStatus();
        if (isContentProtectionActivated) {
    ...
    async checkContentProtectionStatus() {
        try {
            const contentProtectionPlugin = this.app.plugins.plugins["obsidian-content-protection"];
            if (contentProtectionPlugin) {
                const isPluginEnabled = this.app.plugins.enabledPlugins.has("obsidian-content-protection");
                if (!isPluginEnabled) {
                    return false;
                }
                if (contentProtectionPlugin.settings) {
                    const s = contentProtectionPlugin.settings;
                    const baseOk = s.isActivated === true &&
                        s.deviceId &&
                        s.deviceId.trim().length > 0 &&
                        s.hasBeenActivated === true;
                    if (!baseOk) {
                        return false;
                    }
                    const hashOk = await this.verifyContentProtectionHash(s);
                    return hashOk;
                }
            }
    ...
    async checkContentProtectionFile() {
        try {
            ...
            const contentProtectionDataPath = ".obsidian/plugins/obsidian-content-protection/data.json";
            const data = await this.app.vault.adapter.read(contentProtectionDataPath);
            const settings = JSON.parse(data);
            const baseOk = settings.isActivated === true &&
                settings.deviceId &&
                settings.deviceId.trim().length > 0 &&
                settings.hasBeenActivated === true;
            if (!baseOk) {
                return false;
            }
            const hashOk = await this.verifyContentProtectionHash(settings);
            return hashOk;
        }
        catch (error) {
            return false;
    ...
    async checkAndUpdateStatus() {
        const isStillActivated = await this.checkContentProtectionStatus();
        if (!isStillActivated && this.isPluginActivated) {
            this.isPluginActivated = false;
            this.disablePluginFeatures();
        }
        else if (isStillActivated && !this.isPluginActivated) {
            this.isPluginActivated = true;
            await this.initializePluginFeatures();
        }
    }
