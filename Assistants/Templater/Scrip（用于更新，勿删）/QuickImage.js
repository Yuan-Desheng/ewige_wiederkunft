...
class QuickImage {
    ...
    async httpRequest(url, options = {}, timeout = 5000) {
        const ru = typeof requestUrl === 'function' ? requestUrl : (typeof window !== 'undefined' && typeof window.requestUrl === 'function' ? window.requestUrl : null);
        if (ru) {
            const req = ru({
                url,
            ...
            return {
                ...
                status: res.status,
                json: async () => (res.json ? res.json : JSON.parse(res.text)),
                text: async () => res.text
            };
        }
    ...
    async translateToEnglish(text, maxRetries = 3) {
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                const response = await this.httpRequest(translateUrl.toString(), {}, 5000);
    ...
    async makeImage() {
        ...
        try {
            ...
            if (maskClass) {
                return this.tR + prefix + `<span class="${maskClass}" style="width: ${width}px; height: ${height}px;" data-imagen-wrap="true"><img src="${imageUrl}" alt="${keywords}" style="width: 100%; height: 100%; object-fit: cover;"></span>\u200B`;
            } else {
